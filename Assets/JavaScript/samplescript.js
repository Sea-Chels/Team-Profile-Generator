...
const generateInternCard = () =>{
    let internCard = `<div class="card"> 
        ...
        <p><a class="worker-email" href="mailto:${newIntern.email}">${newIntern.email}</a></p>
        <p class="school">${newIntern.school}</p>
    </div> `;
...
const generateEngineerCard = () => {
    let engineerCard = `<div class="card"> 
        ...
        <p><a class="worker-email" href="mailto:${newEngineer.email}">${newEngineer.email}</a></p>
        <p><a class="github" href="github.com/${newEngineer.github}">${newEngineer.gihutb}</a></p>
    </div> `;
...
//to do: write a function that only has the add new employee prompt, then write the async function below, and incorporate it into the init and the employeeAsync functions to try to fix the loop bug. 

async function newEmployee (){

    // var addNewEmp = await function () {
    //     return inquirer.prompt(
    // {
    //     type: 'checkbox',
    //     message: 'Add an Employee?',
...
async function employeeAsync (){
    ...
    console.log(employeeData.addEmployee[0])
    if (employeeData.addEmployee[0] !== "I'm Done"){
        let results = await addEmployee(employeeData);
        console.log(results);
        console.log("Does not equal I'm done");
    }else{
        finishHTMLFile();
